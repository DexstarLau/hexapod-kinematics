...
def test_margin_is_ruled_and_is_a_divisor_on_demand():
    ...
    k = C.load()
    assert k.status("torque_margin") == "decided"
...
def test_the_invariant_closes_exactly_at_the_ceiling():
    ...
    tripod = k.value("payload_mass_kg") / k.value("tripod_support_legs") * a_eff / 10.0
    assert round(tripod, 4) == 8.0
    assert round(tripod * k.value("torque_margin"), 4) == k.value("stall_torque_kgcm")
...
def test_d188s_retired_ceiling_was_a_3x_margin_all_along():
    ...
    k = C.load()
    implied = (k.value("stall_torque_kgcm") * 10.0 * k.value("tripod_support_legs")
               / (k.value("payload_mass_kg") * 93.0233))
...
def test_single_leg_diagnostic_is_emitted_and_is_not_a_failure():
    ...
    ratio = (k.value("payload_mass_kg") * D.a_eff_max_mm(k) / 10.0
             / k.value("stall_torque_kgcm"))
