...
tools/       Scripts that read data this repository does not ship, and the
             generator for the printable targets
print/       Printable targets. Generated, committed, checksummed
data/        Collection manifest template
...
`print/` holds two PDFs, generated by `tools/make_print_set.py` and committed
so they need no toolchain to reprint:

| File | Job |
|---|---|
| `aruco_4x4_100mm.pdf` | 4 range markers, `DICT_4X4_50`, 100 mm nominal |
| `charuco_a4_landscape.pdf` | ChArUco calibration board, `DICT_5X5_100`, 7×5 at 32 mm |

Different dictionaries on purpose: a frame containing both targets would
otherwise report the same marker ID twice.

**Print at 100 %.** Every range estimate divides by the marker's *measured*
width, not by 100 — there is a caliper field on each sheet, and
`tools/check_manifest.py` warns if a manifest records exactly 100.0.

Generation is byte-reproducible and the checksums are in
