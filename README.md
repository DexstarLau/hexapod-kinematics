...
| Derivation and the sweep outputs | working, checked against externally supplied figures. **D415 rules fifteen; fourteen are named** — output 15 (`foot_dz_per_quantum_mm`, two columns) added 16 September, `theta_span_deg` now follows D100, `body_speed_mm_s` is a labelled convenience, not an output. The fifteenth member is not identified and is not guessed |
| Stance pose, P1′ (D414) | working — `sim/stance_pose.py`. Mini Project's own solve of body height, pitch and roll; reproduces D414's recorded figures and plants every stance foot through an independent forward kinematics. Reports two swing-clearance readings and the pose step at each tripod hand-over |
| D197 re-sweep | run — `tools/resweep.py`, table in `reports/d197_resweep.csv`, regenerated by a test on every run. **Nothing is chosen here; coordination rules the pair** |
...
**258 passing and 0 skipped, on CI and on any machine.** Until 12 September six
